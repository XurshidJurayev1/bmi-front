...
export const joinedCourse = (id, user) => async (dispatch) => {
  const res = await api.put(`courses/joined/${id}`, user);
  dispatch({
    type: 'JOINED_COURSE',
    payload: res.data,
  });
...
export const completeCourse = (id, course) => async (dispatch) => {
  const res = await api.put(`users/complete/${id}`, course);
  dispatch({
